...
resource "aws_launch_template" "aws09_was_lt" {
  ...
  network_interfaces {
    ...
    security_groups             = [data.aws_security_group.aws09_was_sg.id]
  }

  iam_instance_profile {
    name = data.aws_iam_instance_profile.aws09_ec2_profile.name
  }
...
resource "aws_autoscaling_group" "aws09_was_asg" {
  ...
  vpc_zone_identifier = data.aws_subnets.aws09_private_subnets.ids
  ...
  target_group_arns = [data.aws_lb_target_group.aws09_was_tg.arn]
